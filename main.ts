...
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    if ((mySprite.tileKindAt(TileDirection.Top, assets.tile`tile4`) || mySprite.tileKindAt(TileDirection.Top, assets.tile`tile5`)) && vertical == -1 || (mySprite.tileKindAt(TileDirection.Right, assets.tile`tile4`) || mySprite.tileKindAt(TileDirection.Left, assets.tile`tile5`)) || (mySprite.tileKindAt(TileDirection.Right, assets.tile`tile6`) || mySprite.tileKindAt(TileDirection.Left, assets.tile`tile7`))) {
        ...
        if (info.score() > 4) {
            if (info.life() < 10) {
                ...
                info.changeLifeBy(1)
                game.showLongText("Your health has been restored by one!", DialogLayout.Bottom)
            } else {
    ...
    } else if (mySprite.tileKindAt(TileDirection.Top, sprites.dungeon.doorClosedNorth) && vertical == -1 || mySprite.tileKindAt(TileDirection.Right, sprites.dungeon.doorClosedNorth) && horizontal == 1 || mySprite.tileKindAt(TileDirection.Left, sprites.dungeon.doorClosedNorth) && horizontal == -1) {
        reward = randint(0, 4)
        tiles.setTileAt(tiles.getTileLocation(23, 0), sprites.dungeon.doorOpenNorth)
        mySprite.setFlag(SpriteFlag.Invisible, true)
        sprites.destroyAllSpritesOfKind(SpriteKind.enemy_unaware)
        ...
        scene.setBackgroundImage(img`
            ...
            `)
        game.showLongText("You escaped the castle!", DialogLayout.Bottom)
        game.over(true)
    } else if (mySprite.tileKindAt(TileDirection.Left, sprites.dungeon.purpleOuterEast0) && horizontal == -1) {
        game.splash("Wow! A door! ")
        game.splash("But it's locked...")
    } else {
        game.showLongText("There's nothing to interact with", DialogLayout.Bottom)
    }
})
...
let actv = 0
game.splash("Castle Crawler")
game.splash("By ZC123")
game.showLongText("You woke up in a strange place with no memory", DialogLayout.Bottom)
game.showLongText("Escape the castle!", DialogLayout.Bottom)
game.showLongText("Press A to attack", DialogLayout.Bottom)
game.showLongText("Press B to interact with things", DialogLayout.Bottom)
game.showLongText("Use the buttons to control the character", DialogLayout.Bottom)
let treasure = [
img`
    ................f...
    ...............f4f..
...
speed = 70
vertical = -1
tiles.placeOnTile(mySprite, tiles.getTileLocation(43, 41))
    ...
    for (let value2 of sprites.allOfKind(SpriteKind.Enemy)) {
        if (mySprite.y < value2.y + 56 && mySprite.x < value2.x + 56 && (mySprite.y > value2.y - 56 && mySprite.x > value2.x - 56)) {
            if (actv > 3) {
                ...
                game.showLongText("A Snake! Run!", DialogLayout.Bottom)
                value2.follow(mySprite, 30)
                actv = 5
            }
        }
...
forever(function () {
    if ((mySprite.tileKindAt(TileDirection.Top, assets.tile`tile4`) || mySprite.tileKindAt(TileDirection.Top, assets.tile`tile5`)) && vertical == -1 || (mySprite.tileKindAt(TileDirection.Right, assets.tile`tile4`) || mySprite.tileKindAt(TileDirection.Left, assets.tile`tile5`)) || (mySprite.tileKindAt(TileDirection.Right, assets.tile`tile6`) || mySprite.tileKindAt(TileDirection.Left, assets.tile`tile7`))) {
        mySprite.sayText("Press B to interact", 500, false)
    } else if (mySprite.tileKindAt(TileDirection.Top, sprites.dungeon.greenSwitchUp) && vertical == -1) {
        mySprite.sayText("Press B to interact", 500, false)
    } else if (mySprite.tileKindAt(TileDirection.Top, sprites.dungeon.doorClosedNorth) && vertical == -1 || mySprite.tileKindAt(TileDirection.Right, sprites.dungeon.doorClosedNorth) && horizontal == 1 || mySprite.tileKindAt(TileDirection.Left, sprites.dungeon.doorClosedNorth) && horizontal == -1) {
        mySprite.sayText("Press B to interact", 500, false)
